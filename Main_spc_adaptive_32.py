...
from conv_LSTM_adaptive_128 import ConvLSTM
...
from torch.optim import lr_scheduler
from utils import Logger
from data_cluster_loader_32 import UCF101, UCF101_val
from conv_LSTM_adaptive import ConvLSTM
from utils import AverageMeter
import torchvision.utils as v_image
from math import log10
from skimage import measure
...
resume_path = './results_raw/save_model_adaptive_32.pth'
result_path = './results_raw'

hdf5_file  = './training_32.hdf5'
hdf5_file_validation  = './validation_32.hdf5'
...
train_epoch_logger = Logger(
    os.path.join(result_path, 'train_adaptive_32.log'),
    ['epoch', 'loss'])

train_batch_logger = Logger(
    os.path.join(result_path, 'train_batch_adaptive_32.log'),
    ['epoch', 'batch', 'loss', 'lr'])
...
for epoch in range(num_epochs):
    ...
    if epoch % checkpoint == 0:
        ...
        for j in range(batch_size):
            for k in range(1):
                v_image.save_image(inputs[j,k,:].data,'%s/x_bar_input_epoch_%04d_%04d.png' % ("./output_image_adaptive_32", j, k))
                v_image.save_image(outputs[j,k,:].data,'%s/x_bar_epoch_%04d_%04d.png' % ("./output_image_adaptive_32", j, k))
    ...
    for j in range(batch_size):
        for k in range(1):
            v_image.save_image(inputs[j, k, :].data,
                               '%s/x_bar_input_epoch_%04d_%04d.png' % ("./output_image_validation_adaptive_32", j, k))
            v_image.save_image(outputs[j, k, :].data,
                               '%s/x_bar_epoch_%04d_%04d.png' % ("./output_image_validation_adaptive_32", j, k))
