...
resume_path = './results_raw/save_model_adaptive_128.pth'
...
train_epoch_logger = Logger(
    os.path.join(result_path, 'train_adaptive_128.log'),
    ['epoch', 'loss'])

train_batch_logger = Logger(
    os.path.join(result_path, 'train_batch_adaptive_128.log'),
    ['epoch', 'batch', 'loss', 'lr'])
...
for epoch in range(num_epochs):
    ...
    if epoch % checkpoint == 0:
        ...
        for j in range(batch_size):
            for k in range(1):
                v_image.save_image(inputs[j,k,:].data,'%s/x_bar_input_epoch_%04d_%04d.png' % ("./output_image_adaptive_128", j, k))
                v_image.save_image(outputs[j,k,:].data,'%s/x_bar_epoch_%04d_%04d.png' % ("./output_image_adaptive_128", j, k))
    ...
    for j in range(batch_size):
        for k in range(1):
            v_image.save_image(inputs[j, k, :].data,
                               '%s/x_bar_input_epoch_%04d_%04d.png' % ("./output_image_validation_adaptive_128", j, k))
            v_image.save_image(outputs[j, k, :].data,
                               '%s/x_bar_epoch_%04d_%04d.png' % ("./output_image_validation_adaptive_128", j, k))
